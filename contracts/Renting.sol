...
contract Renting is IERC721Receiver, Ownable {
  ...
  uint256[] rentableTokens; // List of tokens that can be rented
  ...
  // Get Stake[id] data
  function getStakeDataForId(uint256 id)
    public
    view
    returns (
      uint256,
  ...
  function unstakeAccessPass(uint256 _tokenId) external {
    require(
      ...
      "You do not own this Access Pass."
    );
    require(
      rents[_tokenId].renter == address(0),
      "This is pass is still rented out."
    );
  ...
  function rent(address renter, uint256 tokenId) external payable {
    ...
    require(
      rents[tokenId].renter == address(0),
      "This token is already rented out."
    );
    require(
      stakes[tokenId].price == msg.value,
      "Not enough ether send to pay for rent."
    );
  ...
  function getWalletRentStatus(address renter)
    external
    view
    returns (
      uint256,
    ...
    )
  {
    ...
    require(
      rents[id].renter != address(0),
      "This address has not rented a token"
    );
  ...
  function withdrawRentBalance() external {
    uint256 balance = rentBalance[msg.sender];
    ...
    (bool success, ) = payable(msg.sender).call{ value: (balance * 90) / 100 }(
      ""
  ...
  function withdrawTreasuryCommissionBalance() external {
    uint256 balance = treasuryCommission;
  ...
  function getListOfRentableTokens()
    external
    view
    returns (
      uint256[] memory,
    ...
    )
  {
    ...
    uint256[] memory tokenId = new uint256[](rentableTokens.length);
    uint256[] memory price = new uint256[](rentableTokens.length);
    uint256[] memory rentEndDate = new uint256[](rentableTokens.length);

    for (uint256 i = 0; i < rentableTokens.length; i++) {
      if (stakes[rentableTokens[i]].isRentable) {
        // Check if token is rentable
